...
export async function POST(req: NextRequest) {
  ...
  const { branch_id, name, icon, sort_order, cover_url } = await req.json();
  ...
  const id = createCategory({ branch_id: Number(branch_id), name, slug, icon: icon || '🍽️', sort_order: sort_order || 0, cover_url: cover_url || '' });
